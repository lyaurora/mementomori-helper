...
The browser remembers the last chat channel separately for each account, including after navigation or reload. On a fresh browser connection the conversation appears after preferences load, without briefly showing World first; within the same browser session, cached preferences render the selected channel immediately. It does not restore a private recipient or store message content. The sticker button sits immediately left of **发送**. Its fixed-size panel sits beside wide conversations and below the composer in narrow ones, keeping the input usable while switching categories or inserting multiple stickers.
...
Reaction pickers are instantiated only when opened. Closed messages do not register hidden popovers or perform their JavaScript setup on every guild-channel visit. Initial channel and appearance reads run together; subsequent navigation uses circuit-local state rather than waiting for browser storage again.
